(function () {
  ...
  // Price list from services.js
  const data = window.STELLA_SERVICES || [];
      ...
      <ul class="svc-list">
        ${cat.items.map(([name, dur, cost, note]) => `
          <li class="svc">
            ...
            <span class="svc-lead" aria-hidden="true"></span>
  ...
  // Reveal on scroll
  const revealables = $$('.section-head, .cat, .masonry figure, .tags li, .team-note, .quotes blockquote, .ratings, .know-list > div, .visit-info, .visit-side > *, .salon-strip figure');
  revealables.forEach(el => el.classList.add('reveal'));
  $$('.tags li, .know-list > div, .quotes blockquote, .salon-strip figure').forEach((el, i) => el.style.setProperty('--i', i % 8));
