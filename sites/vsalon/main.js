(function () {
  ...
  // Services menu from services.js
  const data = window.VSALON_SERVICES || [];
  const catNav = $('#cat-nav');
  const menuEl = $('#menu');
  const price = p => {
    if (p === 'consult') return '<span class="consult">Consultation</span>';
  ...
  data.forEach(cat => {
    ...
    sec.innerHTML = `
      <div class="cat-head">
        <h3 class="cat-title">${cat.title}${cat.zh ? `<span class="zh">${cat.zh}</span>` : ''}</h3>
  ...
  // Reveal on scroll
  const revealables = $$('.section-head, .salon-text, .salon-photos figure, .cat, .team li, .team-cta, .masonry figure, .quotes blockquote, .ratings, .faq-list > div, .visit-info, .visit-side > *');
  revealables.forEach(el => el.classList.add('reveal'));
  $$('.team li, .faq-list > div, .quotes blockquote').forEach((el, i) => el.style.setProperty('--i', i % 6));
